...
#[test]
fn value_query_wiki_body_matching() {
    let mut runner = TestRunner::default();
    let ops_and_queries = esmy_test::do_gen(10, 0..50, arb_wiki_doc(), 0..10, {
        |docs| esmy_test::query_gen::value_query(docs, "text")
    });
...
#[test]
fn value_query_wiki_id_matching() {
    let mut runner = TestRunner::new(Config::with_cases(100));
    let ops_and_queries = esmy_test::do_gen(10, 0..10, arb_wiki_doc(), 0..10, {
        |docs| esmy_test::query_gen::value_query(docs, "id")
    });
    let schema = SegmentSchemaBuilder::new()
...
#[test]
fn term_query_wiki_body_matching() {
    let mut runner = TestRunner::default();
    let ops_and_queries = esmy_test::do_gen(10, 0..10, arb_wiki_doc(), 0..10, {
        |docs| esmy_test::query_gen::term_query(docs, "text", Box::from(UAX29Analyzer::new()))
    });
    let schema = SegmentSchemaBuilder::new()
...
#[test]
fn text_query_wiki_body_matching() {
    let mut runner = TestRunner::new(Config::with_cases(1000));
    let ops_and_queries = esmy_test::do_gen(10, 0..10, arb_wiki_doc(), 0..10, {
        |docs| esmy_test::query_gen::text_query(docs, "text", Box::from(UAX29Analyzer::new()), 5)
    });
    let schema = SegmentSchemaBuilder::new()
        .add_string_index("string_index", "text", Box::from(UAX29Analyzer {}))
        .add_full_doc_with_compression("full_doc", 0)
...
#[test]
fn all_query_wiki_body_matching() {
    let mut runner = TestRunner::default();
    let ops_and_queries = esmy_test::do_gen(10, 0..10, arb_wiki_doc(), 0..10, {
        |docs| esmy_test::query_gen::all_queries(docs, "text", Box::from(UAX29Analyzer::new()))
    });
    let schema = SegmentSchemaBuilder::new()
        .add_string_index("string_index", "text", Box::from(UAX29Analyzer {}))
        .add_full_doc_with_compression("full_doc", 0)
...
fn all_docs_many_docs_matching() {
    ...
    let ops_and_queries = esmy_test::do_gen(5, 500..10000, arb_wiki_doc(), 1..10, {
...
fn text_query_name_matching_pos_index() {
    ...
    let ops_and_queries = esmy_test::do_gen(10, 0..10, arb_wiki_doc(), 1..10, {
        |docs| esmy_test::query_gen::text_query(docs, "text", Box::from(UAX29Analyzer::new()), 4)
    });
    let schema = SegmentSchemaBuilder::new()
        .add_string_pos_index("string_pos_index", "text", Box::from(UAX29Analyzer::new()))
        .add_full_doc_with_compression("full_doc", 0)
        .build();
...
fn arb_wiki_doc() -> BoxedStrategy<Doc> {
    (0..WIKI_DOCS.len())
